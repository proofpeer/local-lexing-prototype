...
trait Document extends LaidoutInput {
  
  def size : Int
  
  /** @return (row, column, characterCode) */
  def character(position : Int) : LaidoutChar

  def at(position : Int) = character(position)
  ...
  def print(position : Int, len : Int) : String = getText(position, len)
...
object Document {
  ...
  import net.proofpeer.locallexing.utils.StringUtils._
